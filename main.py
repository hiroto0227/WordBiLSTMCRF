...
def evaluate(data, model, name, nbest=None):
    ...
    for batch_id in range(total_batch):
        ...
        if not instance:
            continue
        batch_word, batch_features, batch_wordlen, batch_wordrecover, batch_char, batch_charlen, batch_charrecover, batch_sws, batch_swlens, batch_swrecovers, batch_swfmask, batch_swbmask, batch_label, mask  = batchify_with_label(instance, data.HP_gpu, False)
        if nbest:
            scores, nbest_tag_seq = model.decode_nbest(batch_word,batch_features, batch_wordlen, batch_char, batch_charlen, batch_charrecover, batch_sws, batch_swlens, batch_swrecovers, batch_swfmask, batch_swbmask, mask, nbest)
...
def batchify_with_label(input_batch_list, gpu, if_train=True):
    ...
    sw_num = len(input_batch_list[0][3])
    sws_list = [sent[3] for sent in input_batch_list]
    sw_fmasks_list = [sent[4] for sent in input_batch_list]
    sw_bmasks_list = [sent[5] for sent in input_batch_list]
    labels = [sent[6] for sent in input_batch_list]
